...
class Graph:
    def __init__(self, points):
        ...
        color1 = (0.0, 1.0, 0.0)  # RGB values for the first color
        color2 = (1.0, 0.0, 0.0)  # RGB values for the second color

        # Generate a gradient of colors for the edges
        cmap = colors.LinearSegmentedColormap.from_list('custom', [color1, color2])
    ...
    def plot(self):
        ...
        for i, edge in enumerate(self.edges):
            u, v = edge
            ax.plot([self.points[u][0], self.points[v][0]], [self.points[u][1], self.points[v][1]], [self.points[u][2], self.points[v][2]])
        plt.show()
